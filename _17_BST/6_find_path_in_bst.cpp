...
// right child, take -1 in its place. Since -1 is used as an indication whether the left or right nodes 
...
vector<int>* getPath(BinaryTreeNode<int> *root , int data) {
    ...
    if(root->data>data){
        vector<int>* lo = getPath(root->left, data);
        if(lo){
            lo->push_back(root->data);
        }
        return lo;
    }
    vector<int>* ro = getPath(root->right, data);
        if(ro){
            ro->push_back(root->data);
        }
    return ro;  
}
