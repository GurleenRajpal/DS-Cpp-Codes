...
// Each test case contains two lines. The first line of each test case will contain two 
// space-separated integers N and K, where N is the size of the given array and K is the target value. 
...
int solve(int index, int sum, int* a, int n, int k){
    ...
    int take = solve(index+1, sum+a[index], a, n, k);
    
    int nonTake = solve(index+1, sum, a, n, k);
